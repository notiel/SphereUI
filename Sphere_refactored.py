...
class SphereUi(QtWidgets.QMainWindow, design_new.Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.green_list: List[QtWidgets.QCheckBox] = [self.CBG1, self.CBG2, self.CBG3, self.CBG4, self.CBG5, self.CBG6,
                                                      self.CBG7, self.CBG8, self.CBG9, self.CBG10, self.CBG11,
                                                      self.CBG12, self.CBG13, self.CBG14, self.CBG15, self.CBG16,
                                                      self.CBG17, self.CBG18, self.CBG19, self.CBG20, self.CBG21,
                                                      self.CBG22, self.CBG23, self.CBG24, self.CBG25, self.CBG26,
                                                      self.CBG27, self.CBG28, self.CBG29, self.CBG31, self.CBG32,
                                                      self.CBG33, self.CBG34, self.CBG35, self.CBG36, self.CBG30]

        # list of blue led checkboxes
        self.blue_list: List[QtWidgets.QCheckBox] = [self.CBB1, self.CBB2, self.CBB3, self.CBB4, self.CBB5, self.CBB6,
                                                     self.CBB7, self.CBB8, self.CBB9]
        ...
        self.mapping: Dict[QtWidgets.QCheckBox, int] = {self.CBG1: 1, self.CBG2: 2, self.CBG3: 4, self.CBG4: 5,
                                                        self.CBG5: 9, self.CBG6: 10, self.CBG7: 11, self.CBG8: 12,
                                                        self.CBG9: 15, self.CBG10: 16, self.CBG11: 17, self.CBG12: 19,
                                                        self.CBG13: 22, self.CBG14: 24, self.CBG15: 25, self.CBG16: 26,
                                                        self.CBG17: 29, self.CBG18: 30, self.CBG19: 32, self.CBG20: 33,
                                                        self.CBG21: 36, self.CBG22: 37, self.CBG23: 38, self.CBG24: 39,
                                                        self.CBG25: 43, self.CBG26: 45, self.CBG27: 46, self.CBG28: 47,
                                                        self.CBG29: 51, self.CBG30: 52, self.CBG31: 53, self.CBG32: 54,
                                                        self.CBG33: 57, self.CBG34: 58, self.CBG35: 59, self.CBG36: 61,
                                                        self.CBB1: 3, self.CBB2: 8, self.CBB3: 18, self.CBB4: 23,
                                                        self.CBB5: 31, self.CBB6: 40, self.CBB7: 44, self.CBB8: 50,
                                                        self.CBB9: 60, self.CBW1: 6, self.CBW2: 7, self.CBW3: 13,
                                                        self.CBW4: 14, self.CBW5: 20, self.CBW6: 21, self.CBW7: 27,
                                                        self.CBW8: 28, self.CBW9: 34, self.CBW10: 35, self.CBW11: 41,
                                                        self.CBW12: 42, self.CBW13: 48, self.CBW14: 49, self.CBW15: 55,
                                                        self.CBW16: 56, self.CBW17: 62, self.CBW18: 63}

        # leds in a matrix for shift effect
        self.matrix: List[List[QtWidgets.QCheckBox]] = [[self.CBG1, self.CBB2, self.CBG9, self.CBG13, self.CBG17,
                                                         self.CBG21, self.CBG25, self.CBB8, self.CBG33],
                                                        [self.CBG2, self.CBG5, self.CBG10, self.CBB4, self.CBG18,
                                                         self.CBG22, self.CBB7, self.CBG29, self.CBG34],
                                                        [self.CBB1, self.CBG6, self.CBG11, self.CBG14, self.CBB5,
                                                         self.CBG23, self.CBG26, self.CBG30, self.CBG35],
                                                        [self.CBG3, self.CBG7, self.CBB3, self.CBG15, self.CBG19,
                                                         self.CBG24, self.CBG27, self.CBG31, self.CBB9],
                                                        [self.CBG4, self.CBG8, self.CBG12, self.CBG16, self.CBG20,
                                                         self.CBB6, self.CBG28, self.CBG32, self.CBG36]]

        self.repeat: bool = False
        self.groups: List[Group] = list()
        self.effects: List[Union[Effect, Command]] = list()
        ...
        for line in self.matrix:
            print(','.join([str(self.mapping[CB]) for CB in line]))
    ...
    def create_turnon_effect(self):
        ...
        start_start: int = self.SpinTOStartFrom.value() if not self.CBTOStart.isChecked() \
            else self.SpinToStartTo.value()
        ...
        if self.CBSmooth.isChecked():
            last_frame = get_last_frame(self.effects)
        else:
            last_frame = None
        self.effects.append(create_turn_on(start_start, start_end, lowest_br, highest_br, period_start, period_end,
                                           used_keys, last_frame))
    ...
    def create_shine_effect(self):
        ...
        length: int = self.SpinShineLength.value()
        self.effects.append(create_shine(start_br, end_br, period_start, period_end, length, used_keys))
    ...
    def create_shift_effect(self):
        ...
        checked_keys = list()
        for line in self.matrix:
            checked_keys.append(list())
            for CB in line:
                checked_keys[-1].append(True) if CB.isChecked() else checked_keys[-1].append(False)
        self.repeat = False
        self.effects.append(
            create_shift_effect(direction, brightness, period, tail, self.CBMoveForward.isChecked(),
                                self.CBMovePause.isChecked(), self.repeat, self.effects, checked_keys))
    ...
    def start_repeat_pressed(self):
        ...
        self.effects.append(Command(command="Repeat", parameter=count))
        self.repeat = True
    ...
    def end_repeat_pressed(self):
        ...
        self.effects.append(Command(command='EndOfRepeat', parameter=None))
        self.repeat = False
        self.dump()
